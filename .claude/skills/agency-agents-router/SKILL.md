...
This skill is the portable entry point for Claude-compatible hosts. Codex uses the mirrored `.agents/skills/agency-agents-router/SKILL.md`; ChatGPT may retrieve the same canonical catalog from the connected `Code-With-Cisco/Council` GitHub repository when specialist context would materially improve a request.
