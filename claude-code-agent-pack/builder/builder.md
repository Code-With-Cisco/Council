...
hooks:
  PreToolUse:
    - matcher: "Edit|Write"
      hooks:
        - type: command
          command: 'powershell.exe -NoProfile -NonInteractive -File "${CLAUDE_PROJECT_DIR}/scripts/gates/builder-write-guard.ps1"'
          shell: powershell
    - matcher: "PowerShell"
      hooks:
        - type: command
          command: 'powershell.exe -NoProfile -NonInteractive -File "${CLAUDE_PROJECT_DIR}/scripts/gates/agent-shell-dispatch.ps1" -AgentType builder'
          shell: powershell
---
